...
def p_cockpit_00121_data(spark, busi_date):
    ...
    # 进行连接和筛选
    result_df = df_t.join(df_a,
                          (df_t["busi_month"] == df_a["busi_month"]) & (df_t["filing_code"] == df_a["filing_code"])) \
        .filter(df_t["busi_month"] == i_busi_month) \
        .select(df_t["busi_month"],
                df_t["filing_code"],
                df_t["product_name"],
    ...
    logging.info("ddw.T_COCKPIT_00121写入完成")
