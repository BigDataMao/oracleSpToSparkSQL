...
def p_cockpit_busi_ana_line_m_data(spark: SparkSession, busi_date):
    """
    经营分析-业务条线-按月落地
    ...
    def insert_and_reload(df):
        ...
        return spark.table("ddw.T_COCKPIT_BUSI_ANALYSE_LINE_M").filter(
    ...
    */
    ...
    df_y = tmp.alias("t") \
        ...
        on=(col("t.Business_Line_Id") == col("t1.Business_Line_Id")),
        how="left"
    ).select(
        col("t.Business_Line_Id"),
        col("t.ASSESS_INCOME"),
    ...
    df_y = tmp.alias("t") \
        ...
        other=tmp2.alias("t2"),
        on=(col("t.Business_Line_Id") == col("t2.Business_Line_Id")),
        how="left"
    ).join(
        other=tmp3.alias("t3"),
        on=(col("t.Business_Line_Id") == col("t3.Business_Line_Id")),
        how="left"
    ).select(
        col("t.Business_Line_Id"),
        col("t.avg_rights"),
    ...
    tmp2 = tmp.alias("t").groupBy("t.Business_Line_Id") \
        .agg(count("t.client_id").alias("TOTAL_ACTIVE_NUM")) \
        .select(col("t.Business_Line_Id"), col("TOTAL_ACTIVE_NUM"))

    tmp3 = tmp1.alias("t").groupBy("t.Business_Line_Id") \
        .agg(count("t.client_id").alias("TOTAL_ACTIVE_NUM")) \
        .select(col("t.Business_Line_Id"), col("TOTAL_ACTIVE_NUM"))
    ...
    df_y = tmp_result.alias("t") \
        .select(
        (col("t.business_line_id") / 10000).alias("business_line_id"),
