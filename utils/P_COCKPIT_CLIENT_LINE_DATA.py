...
from pyspark.sql.functions import sum, substring, regexp_replace, round
...
@log
def p_cockpit_client_line_data(spark: SparkSession, busi_date):
    """
    客户分析落地表(客户分析-业务条线)
    ...
    v_jgx_busi_month = v_busi_month + '01'
    ...
    # TODO: CF_BUSIMG.T_COCKPIT_CLIENT_ANALYSE_LINE为分区表,分区字段为busi_month
    return_to_hive(
        spark=spark,
        target_table="ddw.T_COCKPIT_CLIENT_ANALYSE_LINE",
        df_result=df_m,
        insert_mode="overwrite",
    )

    df_m = spark.table("ddw.T_COCKPIT_CLIENT_ANALYSE_LINE").alias("t") \
        .filter(
    ...
    tmp = spark.table("edw.h15_client_sett").alias("t") \
        ...
        on=(
                col("c.oa_branch_id") == col("d.departmentid")
        ),
        how="inner"
    ).filter(
    ...
    tmp_result = df_m.alias("t") \
        ...
        other=tmp_2.alias("b"),
        on=col("t.business_line_id") == col("b.business_line_id"),
        how="left"
    ).select(
        col("t.business_line_id"),
        (
    ...
    df_jgx = spark.table("ods.t_ds_adm_investor_value").alias("a") \
        .filter(
        regexp_replace(col("a.date_dt"), "-", "").between(v_jgx_busi_month, v_end_date)
    ).join(
    ...
    def cal_qw_gp_gjz_tb(label_id, prefix, df_m_func):
        ...
        df_tmp = spark.table("ods.T_crmmg_HIS_LABEL_CLIENT").alias("t") \
            ...
            other=spark.table("ddw.T_OA_BRANCH").alias("d"),
            on=(
                    col("b.oa_branch_id") == col("d.departmentid")
            ),
            how="inner"
        ).filter(
            col("d.business_line_id").isNotNull()
        ...
        df_y_func = df_m_func.alias("t") \
            ...
            on=col("t.business_line_id") == col("c.business_line_id"),
            how="left"
        ).join(
            other=df_tmp_3.alias("d"),
            on=col("t.business_line_id") == col("d.business_line_id"),
            how="left"
        ).select(
            col("t.business_line_id"),
