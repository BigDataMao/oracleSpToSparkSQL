create or replace procedure cf_busimg.P_COCKPIT_BUSI_ANAL_TAR_LINE_Q(I_MONTH_ID    in varchar2,
  ...
  --模块名称:
  --模块编号:
  --模块描述： CF_BUSIMG.T_COCKPIT_BUSI_ANAL_TAR_LINE_Q  经营分析-业务条线-经营目标完成情况-按季度
...
begin
  ...
  delete from CF_BUSIMG.T_COCKPIT_BUSI_ANAL_TAR_LINE_Q t
  ...
  */
  --初始化数据
  INSERT INTO CF_BUSIMG.T_COCKPIT_BUSI_ANAL_TAR_LINE_Q
    (BUSI_YEAR, BUSI_QUARTER, Business_Line_Id, BUSI_TYPE, BUSI_TYPE_NAME)
  ...
  --更新各个指标的数据
  merge into CF_BUSIMG.T_COCKPIT_BUSI_ANAL_TAR_LINE_Q x
  using (
    with tmp as
    ...
    select t.BUSINESS_LINE_ID,
      ...
      from tmp t) y
        on (x.busi_year = v_busi_year and x.BUSI_QUARTER = y.QUARTER_ID and
           x.business_line_id = y.business_line_id and
