create or replace procedure cf_busimg.P_COCKPIT_CLIENT_RESPONS_DATA(i_busi_date   in varchar2,
  ...
  --模块名称:
  --模块编号:
  --模块描述： CF_BUSIMG.T_COCKPIT_CLIENT_ANAL_RESPONS  客户分析落地表(客户分析-分管部门)
  ...
  --========================================================================================================================
  v_op_object  VARCHAR2(50) DEFAULT 'P_COCKPIT_CLIENT_RESPONS_DATA'; -- '操作对象';
  ...
  v_jgx_busi_date varchar(8);--净贡献日期，净贡献数据特殊，日期为每月1号 20240626

begin
  v_busi_month := substr(i_busi_date, 1, 6);
  v_jgx_busi_date:=v_busi_month||'01'; --20240626
  ...
  delete from CF_BUSIMG.T_COCKPIT_CLIENT_ANAL_RESPONS t
  ...
  --初始化数据
  insert into CF_BUSIMG.T_COCKPIT_CLIENT_ANAL_RESPONS
    (BUSI_MONTH, RESPONS_LINE_ID)
    select v_busi_month as busi_month, t.RESPONS_LINE_ID
  ...
  */
  ...
  using (
    with tmp as
     (select t.fund_account_id, d.RESPONS_LINE_ID, sum(t.rights) as rights
        from cf_sett.t_client_sett t
        left join cf_sett.t_fund_account b
          on t.fund_account_id = b.fund_account_id
       inner join cf_busimg.t_ctp_branch_oa_rela c
  ...
  */
  ...
  using (
    ...
    select t.RESPONS_LINE_ID,
      ...
      left join tmp_result2 b
        on t.RESPONS_LINE_ID = b.RESPONS_LINE_ID
     where t.BUSI_MONTH = v_busi_month) y on (a.RESPONS_LINE_ID = y.RESPONS_LINE_ID and a.busi_month = v_busi_month) when matched then
  ...
  using (
    with tmp as
     (
      --有交易
      select t.fund_account_id, d.RESPONS_LINE_ID
        from cf_sett.t_hold_balance t
        left join cf_sett.t_fund_account b
          on t.fund_account_id = b.fund_account_id
       inner join cf_busimg.t_ctp_branch_oa_rela c
  ...
  INSERT INTO CF_BUSIMG.tmp_COCKPIT_CLIENT_jgx
    ...
    select a.investor_id as client_id,
      ...
      JOIN CTP63.T_DS_ADM_BROKERDATA_DETAIL a2
       ...
       and a2.rec_freq = 'M'
     where replace(a.date_dt, '-', '') between v_jgx_busi_date  and v_end_date  --20240626
  ...
  using (
    with tmp as
     (
      --千万工程客户号
      select d.RESPONS_LINE_ID, a.fund_account_id
    ...
    tmp_0 as
     (select a.RESPONS_LINE_ID, count(1) as client_num
        from tmp a
    ...
    tmp_3 as
     (
      --净贡献
      select /*+leading(t) use_hash(t,a)*/
       a.RESPONS_LINE_ID, sum(t.jgx) as jgx
