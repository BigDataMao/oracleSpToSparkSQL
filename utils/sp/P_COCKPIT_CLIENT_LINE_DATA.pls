create or replace procedure cf_busimg.P_COCKPIT_CLIENT_LINE_DATA(i_busi_date   in varchar2,
  ...
  --模块名称:
  --模块编号:
  --模块描述： CF_BUSIMG.P_COCKPIT_CLIENT_LINE_DATA  客户分析落地表(客户分析-业务条线)
  ...
  --========================================================================================================================
  v_op_object  VARCHAR2(50) DEFAULT 'P_COCKPIT_CLIENT_LINE_DATA'; -- '操作对象';
...
begin
  ...
  delete from CF_BUSIMG.T_COCKPIT_CLIENT_ANALYSE_LINE t
  ...
  --初始化数据
  insert into CF_BUSIMG.T_COCKPIT_CLIENT_ANALYSE_LINE
    (BUSI_MONTH, BUSINESS_LINE_ID)
    select v_busi_month as busi_month, t.Business_Line_Id
      from CF_BUSIMG.T_business_line t
     where t.if_use = '1';
  commit;

  --权益结构
  merge into CF_BUSIMG.T_COCKPIT_CLIENT_ANALYSE_LINE a
  using (
    with tmp as
     (select t.fund_account_id,
       ...
       group by t.fund_account_id, b.client_type, d.business_line_id),
    tmp_last as
     (select t.fund_account_id,
             b.client_type,
             d.business_line_id,
    ...
    tmp_result as
     (select t.business_line_id,
        ...
        from CF_BUSIMG.T_COCKPIT_CLIENT_ANALYSE_LINE t
        left join tmp_1 a
          on t.business_line_id = a.business_line_id
        left join tmp_2 b
  ...
  */
  ...
  using (
    with tmp as
     (select t.fund_account_id, d.business_line_id, sum(t.rights) as rights
        from cf_sett.t_client_sett t
        left join cf_sett.t_fund_account b
          on t.fund_account_id = b.fund_account_id
       inner join cf_busimg.t_ctp_branch_oa_rela c
  ...
  */
  ...
  using (
    ...
    select t.business_line_id,
      ...
      left join tmp_result2 b
        on t.business_line_id = b.business_line_id
     where t.BUSI_MONTH = v_busi_month) y on (a.business_line_id = y.business_line_id and a.busi_month = v_busi_month) when matched then
  ...
  using (
    with tmp as
     (
      --有交易
      select t.fund_account_id, d.business_line_id
        from cf_sett.t_hold_balance t
        left join cf_sett.t_fund_account b
          on t.fund_account_id = b.fund_account_id
       inner join cf_busimg.t_ctp_branch_oa_rela c
  ...
  using (
    with tmp as
     (
      --千万工程客户号
      select d.business_line_id, a.fund_account_id
    ...
    tmp_0 as
     (select a.business_line_id, count(1) as client_num
        from tmp a
    ...
    tmp_3 as
     (
      --净贡献
      select /*+leading(t) use_hash(t,a)*/
       a.business_line_id, sum(t.jgx) as jgx
