create or replace procedure cf_busimg.P_COCK_BUSI_ANAL_TARGET_Q_DATA(I_MONTH_ID    in varchar2,
  ...
  --模块名称:
  --模块编号:
  --模块描述： CF_BUSIMG.T_COCKPIT_BUSI_ANAL_TARGET_Q  经营分析-业务单位-经营目标完成情况-按季度
  ...
  --========================================================================================================================
  v_op_object     VARCHAR2(50) DEFAULT 'P_COCK_BUSI_ANAL_TARGET_Q_DATA'; -- '操作对象';
...
begin
  ...
  v_busi_month := CEIL(to_number(substr(I_MONTH_ID, 5, 2)) / 3); --20240626
  ...
  delete from CF_BUSIMG.T_COCKPIT_BUSI_ANAL_TARGET_Q t
  ...
  */
  --初始化数据
  INSERT INTO CF_BUSIMG.T_COCKPIT_BUSI_ANAL_TARGET_Q
    (BUSI_YEAR, BUSI_QUARTER, OA_BRANCH_ID, BUSI_TYPE, BUSI_TYPE_NAME)
  ...
  --更新各个指标的数据
  merge into CF_BUSIMG.T_COCKPIT_BUSI_ANAL_TARGET_Q x
  using (
    with tmp as
    ...
    select t.oa_branch_id,
      ...
      from tmp t) y
        on (x.busi_year = v_busi_year and x.busi_quarter = y.quarter_id and
           x.oa_branch_id = y.oa_branch_id and x.busi_type = y.busi_type) when
     matched then
      update
