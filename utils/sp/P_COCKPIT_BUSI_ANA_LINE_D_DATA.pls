create or replace procedure cf_busimg.P_COCKPIT_BUSI_ANA_LINE_D_DATA(i_busi_date   in varchar2,
...
begin
  ...
  DELETE FROM CF_BUSIMG.T_COCKPIT_BUSI_ANALYSE_LINE_D T
  WHERE T.BUSI_DATE=i_busi_date;
  COMMIT;

  --初始化数据
  insert into CF_BUSIMG.T_COCKPIT_BUSI_ANALYSE_LINE_D
    (BUSI_DATE, Business_Line_Id)
    select i_busi_date as BUSI_DATE, t.Business_Line_Id
  ...
  --业务指标-期末权益
  --业务指标-期末权益同比
  merge into CF_BUSIMG.T_COCKPIT_BUSI_ANALYSE_LINE_D a
  using (
    with tmp_new as
     (
      --新增客户
    ...
    tmp_yoy as
     (
      --同比日期
      select t.fund_account_id, d.Business_Line_Id, sum(t.rights) as end_rights
        from cf_sett.t_client_sett t
        left join cf_sett.t_fund_account b
          on t.fund_account_id = b.fund_account_id
       inner join cf_busimg.t_ctp_branch_oa_rela c
    ...
    tmp_result1 as
     (select t.Business_Line_Id, sum(t.end_rights) as total_end_rights
    ...
    select t.Business_Line_Id,
     ...
     ) y on (a.Business_Line_Id = y.Business_Line_Id and a.busi_date = i_busi_date) when matched then
      update
