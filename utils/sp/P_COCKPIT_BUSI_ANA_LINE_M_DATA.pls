create or replace procedure cf_busimg.P_COCKPIT_BUSI_ANA_LINE_M_DATA(i_month_id    in varchar2,
...
begin
  ...
  DELETE FROM CF_BUSIMG.T_COCKPIT_BUSI_ANALYSE_LINE_M T
   WHERE T.BUSI_MONTH = i_month_id;
  COMMIT;

  --初始化数据
  insert into CF_BUSIMG.T_COCKPIT_BUSI_ANALYSE_LINE_M
    (BUSI_MONTH, Business_Line_Id)
    select i_month_id as BUSI_MONTH, t.Business_Line_Id
  ...
  */
  ...
  using (
    ...
    select t.Business_Line_Id,
      ...
      from CF_BUSIMG.T_COCKPIT_BUSI_ANALYSE_LINE_M t
      left join tmp b
        on t.Business_Line_Id = b.Business_Line_Id
      left join tmp_new c
        on t.Business_Line_Id = c.Business_Line_Id
     where t.busi_month = i_month_id

     ) y on (a.Business_Line_Id = y.Business_Line_Id and a.busi_month = i_month_id) when matched then
      update
    ...
    with tmp as
     (select d.Business_Line_Id,
       ...
       where t.busi_month between v_begin_month and i_month_id
        and D.Business_Line_Id is not null
       group by d.Business_Line_Id, a.ALLOCA_OA_BRANCH_TYPE)
    select t.Business_Line_Id,
           sum(case
  ...
  */
  ...
  using (
    with tmp as
     (
      --
      select d.Business_Line_Id, sum(t.AVG_RIGHTS) as AVG_RIGHTS
  ...
  */
  ...
  using (
    with tmp as
     (select d.Business_Line_Id, sum(t.xt_income) as XT_COLLA_INCOME
       ...
       GROUP BY d.Business_Line_Id),
    tmp1 as
     (select d.Business_Line_Id, sum(t.ZD_INCOME) as XT_COLLA_INCOME
  ...
  */
  merge into CF_BUSIMG.T_COCKPIT_BUSI_ANALYSE_LINE_M a
  using (select d.Business_Line_Id,
          ...
          where substr(t.done_date, 1, 6) between v_begin_month and
                i_month_id
            and D.Business_Line_Id is not null
          group by d.Business_Line_Id) y
  on (a.Business_Line_Id = y.Business_Line_Id and a.busi_month = i_month_id)
  when matched then
    update
  ...
  */
  ...
  using (
    ...
    select t.Business_Line_Id,
      ...
      left join tmp1 c
        on t.Business_Line_Id = c.Business_Line_Id
     where t.busi_month = i_month_id) y on (a.Business_Line_Id = y.Business_Line_Id and a.busi_month = i_month_id) when matched then
      update
  ...
  using (
    ...
    select t.Business_Line_Id,
      ...
      left join tmp_last t1
        on t.Business_Line_Id = t1.Business_Line_Id) y on (a.Business_Line_Id = y.Business_Line_Id and a.busi_month = i_month_id) when matched then
      update
  ...
  using (
    ...
    select t.Business_Line_Id,
      ...
      from tmp t) y
        on (a.Business_Line_Id = y.Business_Line_Id and
  ...
  using (
    ...
    select t.Business_Line_Id,
      ...
      from tmp t
      left join tmp1 t1
        on t.Business_Line_Id = t1.Business_Line_Id
      left join tmp2 t2
        on t.Business_Line_Id = t2.Business_Line_Id
      left join tmp3 t3
        on t.Business_Line_Id = t3.Business_Line_Id) y on (a.Business_Line_Id = y.Business_Line_Id and a.busi_month = i_month_id) when matched then
      update
  ...
  using (
    with tmp as
     (select t.client_id, d.Business_Line_Id, t.open_date
  ...
  using (
    with tmp as
    --当月有效户
     (select t.client_id, d.Business_Line_Id
        from cf_sett.t_hold_balance t
       ...
       where t.busi_date between v_begin_date and v_end_date
         and D.Business_Line_Id is not null
       group by t.client_id, d.Business_Line_Id),
    tmp1 as
     (
      --同比日期有效户
      select t.client_id, d.Business_Line_Id
       ...
       group by t.client_id, d.Business_Line_Id),
    tmp2 as
     (select t.Business_Line_Id, count(1) as TOTAL_ACTIVE_NUM
        from tmp t
       group by t.Business_Line_Id),
    tmp3 as
     (select t.Business_Line_Id, count(1) as TOTAL_ACTIVE_NUM
    ...
    TMP1 AS
     (SELECT T.Business_Line_Id,
        ...
        left join TMP_PRODUCT_pt d
          on t.Business_Line_Id = d.Business_Line_Id
       WHERE T.BUSI_MONTH = I_MONTH_ID)
    SELECT T.Business_Line_Id,
