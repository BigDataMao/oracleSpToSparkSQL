create or replace procedure cf_busimg.P_COCKPIT_BUSI_ANALYSE_D_DATA(i_busi_date   in varchar2,
  ...
  --========================================================================================================================
  v_op_object  VARCHAR2(50) DEFAULT 'P_COCKPIT_BUSI_ANALYSE_D_DATA'; -- '操作对象';
...
begin
  ...
  DELETE FROM CF_BUSIMG.T_COCKPIT_BUSI_ANALYSE_D T
  WHERE T.BUSI_DATE=i_busi_date;
  COMMIT;

  --初始化数据
  insert into CF_BUSIMG.T_COCKPIT_BUSI_ANALYSE_D
    (BUSI_DATE, OA_BRANCH_ID)
    select i_busi_date as BUSI_DATE, t.departmentid
  ...
  --业务指标-期末权益
  --业务指标-期末权益同比
  merge into CF_BUSIMG.T_COCKPIT_BUSI_ANALYSE_D a
  using (
    with tmp_new as
     (
      --新增客户
    ...
    tmp_yoy as
     (
      --同比日期
      select t.fund_account_id, c.oa_branch_id, sum(t.rights) as end_rights
        from cf_sett.t_client_sett t
        left join cf_sett.t_fund_account b
          on t.fund_account_id = b.fund_account_id
       inner join cf_busimg.t_ctp_branch_oa_rela c
    ...
    tmp_result1 as
     (select t.oa_branch_id, sum(t.end_rights) as total_end_rights

        from tmp_yoy t
       group by t.oa_branch_id)
    select t.oa_branch_id,
           t.stock_end_rights/10000 as stock_end_rights,--20240627
           (case
           ...
           t.NEW_END_RIGHTS/10000 as NEW_END_RIGHTS,--20240627
           (case
             when t.total_end_rights <> 0 then
           ...
           t.total_end_rights/10000 as END_RIGHTS,--20240627
           (case
     ...
     ) y on (a.oa_branch_id = y.oa_branch_id and a.busi_date = i_busi_date) when matched then
      update
