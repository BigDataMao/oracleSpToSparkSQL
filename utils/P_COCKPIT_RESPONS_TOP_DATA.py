...
def p_cockpit_respons_top_data(spark, busi_date):
    ...
    tmp_rank = tmp.alias("t") \
        ...
        col("today_profit"),
        col("remain_transfee"),
        rank().over(Window.partitionBy("t.RESPONS_LINE_ID").orderBy(col("t.fund_in").desc())).alias("rank_no_1"),
        rank().over(Window.partitionBy("t.RESPONS_LINE_ID").orderBy(col("t.fund_out").desc())).alias("rank_no_2"),
        # TODO: 亏损前9名的计算逻辑待核实,而且我没筛选,后面再筛
        rank().over(Window.partitionBy("t.RESPONS_LINE_ID").orderBy(col("t.today_profit").desc())).alias("rank_no_3"),
        rank().over(Window.partitionBy("t.RESPONS_LINE_ID").orderBy(col("t.today_profit").asc())).alias("rank_no_4"),
        rank().over(Window.partitionBy("t.RESPONS_LINE_ID").orderBy(col("t.remain_transfee").desc())).alias("rank_no_5")
    )
    ...
    def write_to_hive(num):
        ...
        tmp_result = tmp_rank.alias("t") \
            ...
            col("t.RESPONS_LINE_ID").alias("RESPONS_LINE_ID"),
    ...
    tmp_rank = tmp.alias("t") \
        ...
        col("done_amt"),
        col("done_sum"),
        rank().over(Window.partitionBy("t.RESPONS_LINE_ID").orderBy(col("t.done_amt").desc())).alias("rank_no_6"),
